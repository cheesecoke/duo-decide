...
import { WELCOME_DECISION } from "@/lib/welcomeDecisionContent";
...
interface WelcomeDecisionCardProps {
	onDismiss: () => void;
}

export function WelcomeDecisionCard({ onDismiss }: WelcomeDecisionCardProps) {
	...
	return (
		<CardContainer
			...
			accessibilityLabel={WELCOME_DECISION.title}
		>
			<CardTitle colorMode={colorMode}>{WELCOME_DECISION.title}</CardTitle>
			<DetailsText colorMode={colorMode}>{WELCOME_DECISION.description}</DetailsText>
			<OptionsList>
				{WELCOME_DECISION.options.map((opt, i) => (
